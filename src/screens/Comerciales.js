...
import comercialesData from '../utils/data/datos.json';

const Comerciales = ({route}) => {
  ...
  const uniqueCategories = ['todos', ...new Set(comercialesData.map(item => item.categoria))];
  ...
  const menuItems = uniqueCategories.map(category => ({
    ...
    imageSrc: category === 'todos' ? require('../utils/assets/Todos.png') : { uri: comercialesData.find(item => item.categoria === category)['icono-cat'] }
  }));
  ...
  const filterCardsByCategory = (category) => {
    ...
    if (category === 'todos') {
      ...
      const filteredData = comercialesData.filter(item => item.categoria === category);
